 ...
 * (UCT). The test is constituted of 50 sets, each set is played as a match of 2 
 * games where after one players switch sides.
 * @author kg687
 *
 */
public class TestMCTSHvsMCTS {
	...
	public static void main(String[] args) throws Exception {
		...
		int montecaroloWinCount = 0, mcuctWinCount = 0, drawCount = 0;
		...
		Player[] players =  {
			new Player("MCTS (UCT)", "mcts_uct", "w", 50),
		...
		for(int testIndex = 1; testIndex <= 100; ++testIndex) {
			...
			while(numberOfMove < totalNumberOfMoves) {
				if(players[currentIndex].getType().equals("mcts_uct")) {
			...
			BufferedWriter output = new BufferedWriter(
					new FileWriter("results_MCTSHvsMCTS.txt", true));
			...
			if(gameOutcome.equals("0")) {
				...
				if(gameOutcome.equals(players[0].getColor())) {
					...
					if(players[0].getName().equals("MCTS (UCT)")) {
						++mcuctWinCount;
					} else {
						++montecaroloWinCount;
					}
					...
					//Increment appropriate counter.
					if(players[1].getName().equals("MCTS (UCT)")) {
						++mcuctWinCount;
					} else {
						++montecaroloWinCount;
					}
		...
		output1.newLine();
		output1.append("Monte-Carlo (UCT) victories: " + mcuctWinCount);
		output1.newLine();
		output1.append("========================================");
		output1.close();
