...
/**
 * This class enables testing Monte-Carlo with Heuristic against randomly 
 * playing AI. The test is constituted of 50 sets, each set is played as a match 
 * of 2 games where after one players switch sides.
 * @author kg687
 *
 */
public class TestMCTSHvsRandomAI {

	/**
	 * Run application. This method creates the frame where game is visualized 
	 * and run entire test case. During the test, after each game is finished it
	 * updates the results to the external file in order to keep track about 
	 * results even when the execution of the program would be interrupted. 
	 * Before running this application make sure there is no previous record of 
	 * matches (this application will append new finding, and therefore they 
	 * might become difficult to distinguish from previous ones). 
	 * @param args
	 * @throws Exception
	 */
	public static void main(String[] args) throws Exception {
		//Counters which keep track of number of wins/draws that occurred.
		int montecaroloWinCount = 0, randomaiWinCount = 0, drawCount = 0;
		...
		Player[] players =  {
			new Player("Random", "ai_random", "w", 0),
			new Player("MCTS+H", "MCTS+H", "b", 100000)
		};
		...
		for(int testIndex = 1; testIndex <= 100; ++testIndex) {
			...
			while(numberOfMove < totalNumberOfMoves) {
				if(players[currentIndex].getType().equals("ai_random")) {
					//AI makes random move.
					LuckyAI lucky = new LuckyAI(board, numberOfMove, 
							totalNumberOfMoves);

					Tuple<Integer, Integer> move = lucky.getMove();
			...
			BufferedWriter output = new BufferedWriter(
					new FileWriter("results_MCTSHvsRandom.txt", true));
			...
			if(gameOutcome.equals("0")) {
				...
				if(gameOutcome.equals(players[0].getColor())) {
					...
					if(players[0].getName().equals("Random")) {
						++randomaiWinCount;
					} else {
						++montecaroloWinCount;
					}
					...
					//Increment appropriate counter.
					if(players[1].getName().equals("Random")) {
						++randomaiWinCount;
					} else {
						++montecaroloWinCount;
					}
		...
		output1.newLine();
		output1.append("Monte-Carlo with H. victory: " + montecaroloWinCount);
		output1.newLine();
		output1.append("Random AI victory: " + randomaiWinCount);
		output1.close();
	} //End of main method.
}
